...
console.log(`foodList.indexOf('삼겹살') :  ${foodList.indexOf('삼겹살')}`);             // -1 -> 없는 값이니 음수
...
// Array.prototype.unshift : 배열의 맨 앞에 요소 추가 -> push랑 반대, 성능상 이슈가 있음
// Array.prototype.shift : 배열의 맨 앞 요소 제거 후 반환 -> pop과 반대
...
// slice(시작인덱스, 종료인덱스 -> 종료값은 반환하지 않는다.) -> start <= x < end, 원본 훼손하지 않음
console.log(`front.slice(1, 3) : ${front.slice(1, 3)}`);                        // CSS,JavaScript
console.log(`front : ${front}`);                                                // HTML,CSS,JavaScript,jQuery

// splice(index, 제거수, 추가값1, 추가값2, ...) -> index의 값을 기준으로 제거수만큼 제거하고 뒤에 넣은 추가값을 추가한다. 원본을 변형한다.
console.log(`front.splice(3, 1, "React") : ${front.splice(3, 1, "React")}`);    // jQuery -> 제거 대상을 반환

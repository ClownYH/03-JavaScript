...
function outer() { // <-- 실행 컨텍스트 이전
    // V.E {x : global x, y : global y}
    // L.E {R.C {x : outer X, y : global Y}}
    ...
    function inner() { // <-- inner 실행 함수 실행 이전
        var x = "inner's local x"; // 변수 은닉화
    ...
    inner(); // <-- outer 실행컨텍스트 중지
}

outer(); // <-- outer 실행컨텍스트 실행
